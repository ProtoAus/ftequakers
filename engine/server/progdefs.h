...
#define HALFLIFEMODEL_FIELDS	\
	...
	comfieldfloat(body,"Halflife model format bodygroup selector. Chooses between the alternative submodels of each bodypart - a barney's holstered/drawn/absent pistol, an hgrunt's head and weapon variants.")	/*FTE_CSQC_HALFLIFE_MODELS*/\
	comfieldfloat(animrate,"Halflife pev->framerate: a multiplier on the playback speed of the sequence named by .frame. 0 (the default, and every entity that never sets it) means 1.0 - normal speed. NEGATIVE plays the sequence BACKWARDS from its end, which is what a node-graph transition walked in reverse needs (see FindTransition in the HL SDK). Named animrate rather than framerate because framerate is a widely used mapper key in GoldSrc maps and binding it to an engine field would silently animate every cycler_sprite and env_sprite at its own key value.")	/*FTE_CSQC_HALFLIFE_MODELS*/\
